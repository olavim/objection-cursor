...
const {getOperations} = require('./utils');
...
class CursorPageOperation extends Operation {
	...
	onAdd(builder, args) {
		if (getOperations(builder, /cursorPage/).length > 0) {
			return false;
		}
	...
	onBefore(builder, result) {
		const orderByOps = getOperations(builder, /orderBy/);

		// Swap orderBy directions when going backward
		for (const op of orderByOps) {
			op.args[1] = (this.before === (op.order === 'asc')) ? 'desc' : 'asc';
		}
	...
	onBuild(builder) {
		...
		this.originalBuilder = builder.clone().clear(/onBuild|runAfter/);
		this.resultSizeBuilder = builder.clone().clear(/runAfter/).mergeContext({[RESULTSIZE_BUILDER]: true});
