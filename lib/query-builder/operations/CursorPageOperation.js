...
const {resolveOrderByOperation} = require('../../utils');
...
class CursorPageOperation extends QueryBuilderOperation {
	...
	onBuild(builder) {
		...
		const item = deserializeCursor(this.orderByOps, this.cursor);
		addWhereStatements(builder, this.orderByOps, item);
	...
	onAfter3(_builder, result) {
		...
		*/
		const first = result.length > 0 ? result[0] : (this.before ? item : null);
		const last = result.length > 0 ? result[result.length - 1] : (this.before ? null : item);

		return this._getAdditionalPageInfo(result)
			.then(additionalPageInfo => ({
				results: result,
				pageInfo: Object.assign(additionalPageInfo, {
					next: serializeCursor(this.orderByOps, last),
					previous: serializeCursor(this.orderByOps, first)
				})
			}));
	}
	...
	clone() {
		...
		clone.orderByOps = this.orderByOps;
 ...
 */
function addWhereStatements(builder, orderByOperations, item, composites = []) {
	if (!item) {
		return;
	}
	...
	const {column, value} = resolveOrderByOperation(orderByOp, item);
		...
		if (orderByOperations.length > 1) {
			this.orWhere(function () {
				for (const op of composites) {
					const {column, value} = resolveOrderByOperation(op, item);
					this.andWhere(column, value);
				}

				this.andWhere(function () {
					// Add where statements recursively
					addWhereStatements(this, orderByOperations.slice(1), item, composites);
				});
			});
		}
	});
