...
class CursorPageOperation extends QueryBuilderOperation {
	...
	onBuild(builder) {
		...
		if (this.before) {
			builder.forEachOperation(/orderBy/, op => {
				op.args[1] = (op.args[1] || 'asc') === 'asc' ? 'desc' : 'asc';
			});
		}
	...
	onAfter1(builder, results) {
		// Operations weren't cloned properly in v0, so we need to restore orderBy directions when going backward
		if (this.before) {
			builder.forEachOperation(/orderBy/, op => {
