...
module.exports = function (options, Base) {
	return class extends OrderQueryBuilder(Base) {
		...
		_getOrderByOperations(before = false) {
			...
			return orderByData.map(({column: columnName, order = 'asc'}) => {
				const explicit = orderByExplicitData[columnName];
				return {
					column: explicit ? explicit.column : columnName,
					property: explicit ? explicit.property : columnToProperty(this.modelClass(), columnName),
					// If going backward: asc => desc, desc => asc
					order: before === (order.toLowerCase() === 'asc') ? 'desc' : 'asc',
					getValue: explicit ? explicit.getValue : null
				};
			});
		}
