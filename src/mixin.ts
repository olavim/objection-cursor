...
export type Mixin<T extends AnyFunction> = InstanceType<ReturnType<ReturnType<T>>>;
...
interface CursorPageResultType<R = any> {
	results: R;
	pageInfo: PageInfo;
}
...
interface CursorQueryBuilder<M extends Model, R = M[]> extends QueryBuilder<M, R> {
	...
	orderByCoalesce(col: ColumnRef, dir?: OrderByDirection, coalesceValues?: ColumnRef | ColumnRef[]): this;
	cursorPage(cursor?: string, before?: boolean):
		R extends CursorPageResultType
			? this
...
interface CursorInstance<M extends Model> {
	QueryBuilderType: CursorQueryBuilder<this & M, this[]>;
}

interface CursorStatic<M extends Model> {
	QueryBuilder: any;
	new (...args: any[]): CursorInstance<M> & M;
}
...
export default function mixin(options: Options) {
	return function <T extends AnyConstructor<Model>>(Base: T): CursorStatic<InstanceType<T>> & T {
		class CursorQB<M extends Model, R = M[]> extends QueryBuilder<M, R> {
			public ArrayQueryBuilderType!: CursorQB<M, M[]>;
			public SingleQueryBuilderType!: CursorQB<M, M>;
			public NumberQueryBuilderType!: CursorQB<M, number>;
			public PageQueryBuilderType!: CursorQB<M, Page<M>>;
			...
			public cursorPage(cursor?: string, before: boolean = false) {
				...
				type BuilderType = R extends CursorPageResultType
					? CursorQB<M, R>
					: CursorQB<M, CursorPageResultType<R>>;
			...
			private _getAdditionalPageInfo(models: R[], builder: this, before: boolean) {
				...
				const isEnabled = (opts: Array<keyof typeof options.pageInfo>) => opts.some(opt => options.pageInfo[opt]);
		...
		class Cursor extends Base {
			public static QueryBuilder = CursorQB;
			public QueryBuilderType!: CursorQB<this, this[]>;
		}

		return Cursor as CursorStatic<InstanceType<T>> & T;
	};
}

export type CursorModel = ReturnType<ReturnType<typeof mixin>>;
